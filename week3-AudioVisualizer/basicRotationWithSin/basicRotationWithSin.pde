...
float rotation, radius; // rotation : this is basically how far round the circle we are going to go
                        // radius : this is basically how far from the origin we are.
...
void draw() {
  ...
  for (int i = 0; i < elements;i++) {
      ...
      ellipse(0,0,20,20); // CIRCLES, comment out for scaling trajectories
      //ellipse(0,0,2*i,2*i);
